...
# Linear models with continuous explanatory variables
...
## Linear model of caterpillar growth data
...
d) There was a negative relationship between tannin concentration (mg) and caterpillar growth rate (mg/day) (t = -1.217, p < 0.001) with 78.9% of the variation in growth rate explained by tannin concentration.
...
# Linear models with categorical explanatory variable
...
Run a linear model and check your results. **Hints**:
...
This is a large dataset, so produce a frequency histogram of the residuals from your model, and a qqplot. Notice that in the frequency histogram graph, we've changed the horizontal x-axis label. (Ask a demonstrator if you want to put the two plots side-by-side on one window, but this is options.) **Hints**:
...
Do a multiple comparison test to see which species differ from which. **Hint**: use the `TukeyHSD()` function with your `penguin_lm` model:
...
Again, think carefully about how you would include an explanation. Which species are different from each other when you do pairwise comparisons?
