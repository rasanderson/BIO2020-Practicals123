...
* Create a new R script: in RStudio select **File -> New File -> R Script**. It is good practice to save your script immediately by going to **File -> Save** and give it a sensible name, e.g. `Practical_2.R`.
...
Let's start by calculating some summary statistics for each type of soil, remember that the easiest way to do this is to use functions from the `mosaic` package. 
To do this you will need to "load" the package using `library(mosaic)` function. If you successfully managed to install my `bio2020` package then the line `library(bio2020)` will install all the other packages you need.
...
# A new data set for you to explore if you have time or want more practice
