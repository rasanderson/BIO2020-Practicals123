...
- Check our data `r emo::ji("heavy_check_mark")`
...
We are going look at some data that contains information about penguins that was collected by researchers at the `r emo::ji("penguin")` [palmer research station](https://pal.lternet.edu/)  `r emo::ji("penguin")`  
...
<div id="data-check-hint">
**Hint** The object of the functions is the `penguins` dataset so your code needs to read `head(penguins)` 
</div>
...
We could  do this manually for a chosen variable using the `mean()` function (`r emo::ji("package")` mosaic)
...
An easy way to do this is to use functions (such as `mean()`) from the `mosaic` package with the formula: 
...
Summary stats are a great starting point but to really understand what is going on with our data we need to visualize it. 
...
mean(x~dataset, data = datasaurus_dozen, na.rm=TRUE)
mean(y~dataset, data = datasaurus_dozen, na.rm=TRUE)
# mean and sd of y values in each group
sd(x~dataset, data = datasaurus_dozen, na.rm=TRUE)
sd(y~dataset, data = datasaurus_dozen, na.rm=TRUE)
```
...
There is a lot going on here, by colouring the points in each dataset differently we can see that there look to be some patterns but there is too much information plotted on top of other information to be of much use. We can make life easier by separating out the different datasets into separate windows (we call these facets) to get a better idea of what is going on.
...
The below images are examples of the graphics that can be made entirely using R

```{r}
knitr::include_graphics("images/cedric-wwcup.png")
```

```{r}
knitr::include_graphics("images/cedric-coffee.png")
```
 
 
Although we are going to be starting with simple plots they have the same building blocks as those we have just seen.

For plotting we will use functions from `ggformula` (`r emo::ji("package")`) which allow us to create powerful plots with simple code. 
...
<div id="penguin-density-hint">
**Hint** for a density plot we can use the `gf_density()` function 
</div>
...
You have just created a really sophisticated and informative plot `r emo::ji("tada")`

```{r, echo = FALSE}
knitr::include_graphics("https://media.giphy.com/media/VvXg0yjJQgfEQ/giphy.gif")
```
...
The `alpha` argument alters the transparency of the points, this is useful when we have lots of points that are close together as it helps us to see where points are plotted on top of one another.
...
We use `scale_color_manual()` to create a custom colour pallet, here we have specified three colours by [name](https://www.r-graph-gallery.com/42-colors-names.html)

Add in a note about checking colour blind friendliness of colours


`gf_theme()` is a function that allows us to alter the overall [theme](https://ggplot2.tidyverse.org/reference/ggtheme.html) of the plot, most noticeably it has removed the grey background. 

The other new thing that you may have spotted in the above code is what we call the pipe ` %>% `
...
To make the plot a bit more informative lets add the  data points in as an extra layer, to do this we are going to use the `gf_sina()` function and the pipe. 
...
Can you convert the boxplot to a [violin plot](https://www.data-to-viz.com/graph/violin.html) by altering the code below
...
Now see if you can modify the faceted plot to change the theme and the colours, check the code from the bill length vs. bill depth plot if you get stuck. 

```{r custom, exercise = TRUE, exercise.completion= FALSE}
gf_violin(body_mass_g~species, colour = ~species, alpha = 0.5,data = penguins) %>% 
  gf_sina(shape = ~sex, colour = ~species, alpha = 0.3) %>% 
  gf_labs(x = "Penguin species", y = "Body mass (g)")
```

You have now successfully created some informative plots that you could put straight into a report about penguins such as the [Paper](https://journals.plos.org/plosone/article?id=10.1371/journal.pone.0090081) that these data were originally published in.
