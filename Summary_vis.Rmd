...
Summary stats are a great starting point but to really understand what is going on with our data we need to visualize it. We are using data from the `datasauRus` R package, which has data with identical means, but very different appearances. It shows the imporance of plotting data.
...
You can see that the dataset contains three columns, a `dataset` column, with 142 records in each, and continuous variables `x` and `y`. We will now calculate some summary statistics for `x` and `y` using `dataset` as a grouping variable.
...
It appears that for each `dataset` the summary statistics are the same for `x` and `y`. Every `dataset` group seems to have the same mean and standard deviation for the `x` and `y`. Is this really a good reflection of what the datasets are like?? 

Let us plot them all in one plot:
