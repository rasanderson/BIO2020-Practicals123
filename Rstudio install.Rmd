...
R is both a computer software and a programming language that can be used for powerful data analysis and will be used across several of the taught modules that you will undertake. R is now widely used in Government bodies (e.g. NHS, Natural England), media organisations (BBC, The Guardian, Financial Times), research institutes (Centre for Ecology & Hydrology, Francis Crick Institute) as well as academia. This is because of its flexibility, powerful data analytics and visualisation capabilities. It has a steep initial learning curve (don't panic!) but then becomes much easier. Graduates with strong data science skills in using R are in high demand by employers so it is also a useful addition to your CV.
...
```{r, echo = FALSE, fig.width=8}
knitr::include_graphics("images/file_import_first_error.png")
```
...
```{r, echo = FALSE, fig.width=8}
knitr::include_graphics("images/save_as_excel.png")
```
...
```{r, echo=FALSE}
library(bio2020, warn.conflicts=FALSE, quietly=TRUE, verbose=FALSE)
```
