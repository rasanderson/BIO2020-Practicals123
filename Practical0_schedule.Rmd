...
### How to install and use a package from CRAN
R has a special function called `install.packages()` that allows you install a package onto your PC. On an NUIT computer it will install it into a networked folder, so it should be available for you on any NUIT computer. We'll begin by installing a package called `remotes` which we will use shortly.
...
As the `bio2020` package is solely for use by Newcastle University students it is unsurprisingly not available on CRAN. Instead, it is on a website called GitHub, which is widely used by scientists to manage their software. I store all my analytical data files at [https://github.com/](https://github.com/). If you want to view my software, search on GitHub for the user `rasanderson`. When you typed `library(remotes)` a few minutes ago, you made its extra commands available for use, including the function `install_github` which allows you to install packages from GitHub rather than CRAN. Enter the following at the Console prompt, hitting the Enter key as usual. (Note that the spelling of the function is `install_github` not `install.github`):

```{r, eval=FALSE}
install_github("rasanderson/bio2020")
```

If a message is displayed that says `These packages have more recent versions available` followed by a table of packages, simply hit the Enter key again to skip the update. You can always update them later. A large amount of text is then likely to be displayed as various extra packages are installed. There may be some `Warning` messages, especially about R versions: don't worry about these. If you receive an `Error` message then there is a problem (see below). After you have installed the `bio2020` package you can activate it with the `library` command:
...
If that is displayed then the `bio2020` package has been installed successfully.

### What to do if you are unable to install `bio2020` successfully
Often the `bio2020` package does not successfully install (I am a biologist, not a computer programmer!). Don't worry, we can easily work around it:

* First, double-check with myself or a Demonstrator. Sometimes there are minor differences in the configuration of NUIT PCs within a cluster room that can cause problems, or if many students (i.e. over 150 in this class) overload the network.
* Second, if you are unable to resolve the problems, if you are using an NUIT PC some of the key packages you will need should already have been installed. Try issuing the following commands (don't worry about Warning messages, but look out for Error messages):
...
* Third, if those commands give an Error, or if using your own MacBook or Laptop (**unsupported**) then issue:
...
These are all standard CRAN packages so should install without problems. However, you will have to activate them individually with their own `library` calls, rather than by using a call to `bio2020`:
...
Another package, `vegan` is needed for the last two practicals later in the Semester, but hopefully we will have the `bio2020` package working for you by then.
...
# Activate the bio2020 package so extra commands available
library(bio2020) # or library(mosaic) if bio2020 did not install
...
Now save your R script. Click on **File -> Save** from the RStudio main menu, or if you prefer the save icon, and save the script as `practical0.R`. Next run each line of your script. Click on the `library(bio2020)` line and hit the **Ctl-Shift-Enter** keys simultaneously. This key combination sends a line from your script to the Console window where it is executed. The command should run without displaying anything as `bio2020` is probably already activated. Your cursor should jump to the `read.csv` line. (If you had problems with the `bio2020` package, simply type `library(mosaic)` instead).
...
What has gone wrong? We've just downloaded the file from Canvas yet R claims it is missing? The problem is that when you downloaded the data file from Canvas, it placed it into your **Downloads** folder. You need to move it to the **BIO2020** folder, that already contains your project settings file, and your `practical0.R` file:
...
* Navigate to your BIO2020 folder, Right-click and select **Paste**
...
So, finally we need to convert our Microsoft Excel file. Go back to your File Manager, navigate to the BIO2020 folder (it might already be open) and double-click on the `yield` file to open it in Microsoft Excel. Then click on the **File -> Save As** option, and in the drop-box below the filename, select the CSV option:
...
knitr::include_graphics("images/available_files.png")
```
...
* `BIO2020` which is actually `BIO2020.RProj` and contains your RStudio project settings
...
Look very carefully and you can see that the icons for the two yield files are slightly different, as one has **,a** in it. You can change the settings in File Manager so that full file names with the final "extension" are showing. For the rest of the BIO2020 course all the files will be CSV format, so you will not have this problem. When you enter your own data into Excel for use in RStudio, you need to be aware of it.

Go back to RStudio. Now you should be able to re-run your `read.csv` line. If it is successful, nothing will be displayed in the console, but in your Environment window (top right panel) you should see:
...
You can store them either in your main project folder. Sometimes if you have lots of data files this can get confusing, so I generally recommend creating a folder called **`Data`** inside your `BIO2020` project folder, and putting your CSV files there. The only difference is you need to change the `read.csv` command from:
...
My preference (and recommendation to you) is to put all your CSV files into a `Data` subfolder in your `BIO2020` folder, in which case you'll need to remember to add `Data/` to any `read.csv` commands. That way your BIO2020 folder is less likely to get cluttered as you go through the module.
...
### Books, online guides
See the Canvas reading list for BIO2020. I have updated this with new resources for 2022, with most books in electronic format. There are also lots of excellent resources out there for learning R, some of those that we recommend are listed here and should be enough to keep you going 
