...
Let us build on the example at the [Interactive Website on Curve fitting](https://naturalandenvironmentalscience.shinyapps.io/multiple_explan/#section-fitting-curves) for growth of the grass _Lolium perenne_ in soils of different water content. Dowload the file `grass_growth.csv` from Canvas, save it in your `Data` folder, and import it into R using `read.csv()` as a table called `lolium_dat`. Use the `head()` function to look at the first few rows.

Let's look at a simple linear model to begin. This model is of the form:
...
So that was easy! You have a highly significant linear model, showing that _Lolium_ growth increases with water content. The overall model is significant with $F_{1,17}=11.45, p=0.004$, and the $R^2=0.367$ (model explains 36.7% of variation). But... we should always do a quick plot of the data, and check model assumptions by looking at the residuals. We haven't done either of these. First, create a scatter plot of the data similar to the following (I will leave it to you to remember the correct code):
...
It is immediately obvious that whilst the growth rate increases at first, beyond about 75% soil water it starts to drop sharply. _Lolium_ is not an aquatic plant, so this makes sense of course, but we have not considered it in the model. Let's also check the residuals. We can actually produce 4 "diagnostic" plots quite easily with the `plot()` function. Only the first two are really useful here. The `plot()` function extracts the residuals and creates a QQ plot automatically. All the plots look at "Residuals". Remember that these are the differences between your fitted straight line and the observed points. If you modify your plot, by adding a line for the `gf_lm()` function, the problem is obvious
...
* The first plot is called **Residuals vs Fitted**. Notice how it bends down at both low and high fitted values, with the residuals drifting away from zero, which is ideally where they should scatter.
...
Thus, by looking at the raw data, especially when you overlay the fitted line, as well as the residuals from the model, as well as basic plant biology, you can see this is not a good model. Let's change it to a curve, by changing it to a quadratic. This means our model is now of the form:
...
This linear model is also significant. You will notice that the overall model is highly significant with $F_{2,16}=64.37,p<0.001$ and $R^2=0.876$, or 87.6% explained. Is this improvement in the overall model of `lolium_lm2` compared to the earlier `lolium_lm1` significant? We can compare the two models using `anova()`:
...
### 3.3.2 Diagnostics and plots from quadratic model `lolium_lm2`
...
# 5. Explore some of these concepts with new datasets
Here we give you the opportunity to investigate some of these ideas with some different datasets, that are not on the interactive websites.

## 5.1 Animal weights
Download the file `Pantheria.csv` from Canvas, which contains information from the [Pantheria database](https://ecologicaldata.org/wiki/pantheria). 
You have data which details records of weight,head-body length, skull length, home range and density for species from across several families of the order Carnivora. As usual, save this in your **`Data`** folder, and import it into R/RStudio using the `read.csv()` function. 
...
Before you analyse your data, do a quick `summary()` to show you the names of the columns. You will see that two columns, `Common_name` and `Family` are listed as `Class :character`. Whilst not essential, you might find it easier to change these into "factors" and re-run the `summary()` function. Look back to Part 1.3.1 where you re-coded variables from the pea growth dataset to remind yourself how to do this. Check how many rows of data you have. You can see this is quite a big dataset.
...
In such instances we can use the `filter` function (`r emo::ji("package")` dplyr) to extract only the subset that we are interested in. The `dplyr` package is automatically loaded for you by `mosaic` or by `bio2020`.
...
**Note:** You might be wondering why it says `Family == "Canidae"` rather than `Family = "Canidae"`. The double-equals `==` symbol is used for comparisons. Other useful comparisons might include:

* `!=` **not equal to**. So `Family != "Canidae"` would return all the records except dogs.
* `>`  **greater than**. So `Skull_max > 12` would return larger animals, from all taxa, with a skull size greater than 12 cm.
* `<`  **less than**. So `HomeR_max < 20` would return animals, from all taxa with a small home range
* `>=` and `<=` represent **greater than or equal to** and **less than or equal to** respectively
* `&` **and** you can use this to link things together, e.g. to get dogs, with skulls less than 15 cm:

`small_dog_dat <- filter(pantheria, Family == "Canidae" & Skull_max < 15)`
   
With a big set of data such as `pantheria` you might also want to Click on its name in the **Environment** window (top-right of RStudio) and scroll down the data. It looks a bit like a spreadsheet, although you cannot edit it.
 
Things to try:

* Choose a family to investigate, create a new data frame containing only records from that family using the `filter()` function
...
Now return to the full `panthera` data set and investigate the relationship between **body size** and **weight**.
...
Imagine you were writing a report, based on what you've done. Write a paragraph describing your results, include references to your model statistics and any plots that you think are useful. How would you report the statistics? Which plots and analyses might be best to include. **Hint** just because you create an exploratory plot or do a preliminary analysis does not mean it would have to go into a final report.


## 5.2 Soil bacteria and shrub growth: Interactions between explanatory variables
...
It would be useful to add an interaction term to the model if we wanted to test the hypothesis that the relationship between the amount of bacteria in the soil on the height of the shrub was different in full sun than in partial sun.
 ...
 You may have noticed that the data contained in the `Bacteria` and `Sun` variables are **coded numerically but are in fact categorical**. This is **VERY IMPORTANT TO SPOT!** 
 
 + Re-code the variables so that R recognizes them as categorical rather than numerical using `as.factor()`
...
```
 ...
 + Are the model assumptions, in terms of QQ plots reasonable?
 + Imagine you had done this type of analysis as part of another module or for your Stage 3 project. What would you include in your summary of your results, including model outputs, whether the model assumptions have been met and which figures might you include. 
