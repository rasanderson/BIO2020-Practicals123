...
2. create of some simple summary plots to aid interpretation
...
# Start RStudio, open your BIO2020 project, create R script
...
* Inside RStudio on the main menu click on **File -> Recent Projects -> BIO2020**
* On the RStudio main menu click on **File -> New File -> R Script**

This should open a blank R script at the top left of your screen. Before you go any further, I suggest you type into your script a couple of lines of comments to explain what the script is being used for. As you will be using the `bio2020` R package you should also activate it. (If for any reason you were unable to install `bio2020` then use `mosaic` instead, but speak to a demonstrator first):
...
Position your cursor on `library(bio2020)` and hit the  <kbd>Ctrl</kbd> + <kbd>Enter</kbd>  key combination to send the command into the Console screen so that R executes it. You are likely to get several messages, but hopefully will see "Welcome to the BIO2020 package to support teaching". Finally, before you go further, save your R script. 
...
* Navigate to your "BIO2020" folder, right-click in a blank area inside the BIO2020 folder and select "Paste"
...
* `BIO2020` (full name `BIO2020.RProj`). Your RStudio Project settings created in the previous practical
...
bivalves_dat <- read.csv("bivalves.csv")
```
...
Save your script by selecting **File -> Save** from the main RStudio menu. Next, position your mouse cursor on the first of the three `read.csv` lines, and hit the <kbd>Ctrl</kbd> + <kbd>Enter</kbd> key combinations several times to send the commands to the RStudio Console where they will be run. If you receive an error message, check you've copied the CSV files over from Downloads to your BIO2020 project folder, look for any spelling errors (R is case-sensitive too), check you've typed `read.csv` and not `read-csv`, make sure you've surrounded the CSV filenames with opening and closing `"` symbols, and finally ask a demonstrator.
...
+ **diet** = the column describing which diet the chicks were fed. This is your **_explanatory_** variable as it should be obvious that diet might or might not determine chick weight, but the reverse is cannot true.
...
Please look at [this explanation of variance, or var](https://naturalandenvironmentalscience.shinyapps.io/variation/#section-variance-correcting-ss-for-the-number-of-records) and [this explanation of standard deviation, or sd](https://naturalandenvironmentalscience.shinyapps.io/variation/#section-standard-deviation-correct-units) that introduce these measures of spread. If you do not understand variance and standard deviation, please speak to myself or pop a question into the relevant Microsoft Teams channel at this hyperlink: [Section 1 - summarising and displaying data](https://teams.microsoft.com/l/channel/19%3ab07e8c62b95f42fdb56497d54fe0e1d2%40thread.tacv2/Section%25201%2520-%2520summarising%2520and%2520displaying%2520data?groupId=d8c4adb1-b8dd-4f84-b57d-e4ed03b69652&tenantId=9c5012c9-b616-44c2-a917-66814fbe3e87)  Calculation is straightforward in R, using the functions `var()` and `sd()` and substituting them in as our "goals":
...
Now you have a data frame `diet_stats` that contains summary statistics describing your dataset that might be useful in a report. 
...
```{r, echo = TRUE}
# Create a bar chart showing numbers of birds in each diet
gf_bar(~diet, data = diet_dat) %>% # Plot bar chart THEN
  gf_labs( y = "Number of chicks (n)", x = "Experimental Diet") # Modify axis labels
```

Finally, we will add an additional chain symbol `%>%` to modify or "refine" the "theme" or overall appearance using `gf_theme()`. Personally I like the "classic" theme, `theme_classic()`, which is a simple black-and-white one, although you might prefer the "minimal" theme, `theme_minimal()`:


```{r, eval = TRUE}
# creating the bar chart 
gf_bar(~diet, data = diet_dat) %>%  # Plot bar chart THEN
  gf_labs( y = "Number of Chicks (n)", x = "Experimental Diet") %>% # Modify axis labels THEN
  gf_theme(theme_classic()) # Change graph theme to remove grey backdrop
```
...
At the moment the axis labels are just `weight` and `Diet`. Try changing them into something more meaningful by "chaining" on an additional line that begins `gf_labs()`, so the vertical axis label reads "Weight (g)" as you should always include units where possible.
...
R has its own built in help for functions. These are difficult to understand at first, but as the format is identical in every case they slowly become less daunting. To access the in-built help, put a `?` symbol into the console, followed by the name of the function. For example, in the R package `bio2020` I have written a function called `ordi_identify()` which you will learn how to use in Practical 6. If you wanted to view its help, you would type **in the Console window** (bottom left of RStudio) the following, and hit the <kbd>Enter</kbd> key:

```{r, eval=FALSE}
?ordi_identify
```

# Optional extra - bivalves data
Are you really enjoying R (don't scream...)? If so, do you want to learn how to create a graph like this?
...
This hows the mean numbers of bivalves (+/- standard error bars) at each site, colour-coded for each treatment. To be honest, boxplots are generally my preferred method of displaying data as they show skew better, but as this type of bar-plot is so common, you may need to know how to create it, especially if requested on other modules etc. To create a boxplot is very straightforward: simply set abundance as your response, site as your explanatory, and colour of boxes to be transect, and you know how to edit axis labels, plot themes etc. if needed:

```{r}
gf_boxplot(abundance ~ site, colour= ~transect, data=bivalves_dat)
```

Creating the bar chart is more complicated. You need to make a table (data frame) of 8 rows, one for each bar, that contains the mean abundance, the standard error, the transect, and the site code:
...
Check the contents of your `bivalves_stats` table to make sure it matches what is displayed above. We will use a stepwise approach to building your bar chart. First, a simple barplot:
