...
+ **group** = the name of the column containing information categorical explanatory variable by which we want to group the data
...
Remember you will need to use the pipe symbol `%>%` to join multiple `gf_` functions together.
...
growth_dat <- read.csv("Data/Chick_growth.csv")
 gf_line(Weight~Time, colour = ~Diet, data = growth_dat) %>% # THEN
  ...
  gf_refine(scale_colour_manual(values = c("blue", "red", "orange", "green"))) # define colours
```
